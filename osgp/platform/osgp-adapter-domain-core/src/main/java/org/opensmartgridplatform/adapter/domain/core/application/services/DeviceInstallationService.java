...
public class DeviceInstallationService extends AbstractService {
	...
	public void handleGetOrmStatusResponse(final DeviceEventDto deviceStatusDto, CorrelationIds ids,
            final String messageType, final int messagePriority, final ResponseMessageResultType deviceResult,
            final OsgpException exception) {

        LOGGER.info("handleResponse for MessageType: {}, deviceId: {}, description: {}", messageType, deviceStatusDto.getDeviceIdentification(), deviceStatusDto.getDescription());
        final GetStatusResponse response = new GetStatusResponse();
		response.setOsgpException(exception);
		response.setResult(deviceResult);
		
        Event event = null;
        Device device = this.deviceRepository.findByDeviceIdentification(deviceStatusDto.getDeviceIdentification());
        
        List<Event> events = this.eventRepository.findByDevice(device);
        String previousDescription = null;
        
        if (events.size() > 0) {
        	event = events.get(0);
        	previousDescription = event.getDescription();
        	this.eventRepository.delete(event);
        }
        
        event = new Event(device, new Date(), EventType.MONITOR_EVENTS_REMOTE_MODE, deviceStatusDto.getDescription(), 0);     
